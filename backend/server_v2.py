...
monitors: List[EnvironmentalMonitor] = [
    NTIEnvironmentMonitor("davis339a", "Davis 339A", "https://temp-davis-339a.cse.buffalo.edu/",
                          secrets.NTI_USERNAME, secrets.NTI_PASSWORD, -1.0),
...
class HistoryHandler(BaseHandler):
    def get(self, monitor, days, max_records):
        self.write(Record.get_json_for_monitor(monitor, int(days), int(max_records)))
...
async def main():
    ...
    app = tornado.web.Application([
        ...
        (r"/history/([a-zA-Z0-9_]+)/([0-9]+)/([0-9]+)", HistoryHandler),
